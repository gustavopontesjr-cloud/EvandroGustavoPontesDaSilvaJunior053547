...
export const tutorService = {
  ...
  uploadPhoto: async (id: number, file: File) => {
    ...
    if (!response.ok) throw new Error('Falha ao enviar foto do tutor');
    return response.json();
  },

  // --- NOVAS FUNÇÕES (VÍNCULO) ---
  
  // Vincular Pet (POST /v1/tutores/{id}/pets/{petId})
  linkPet: async (tutorId: number, petId: number) => {
    await api.post(`/v1/tutores/${tutorId}/pets/${petId}`, null);
  },

  // Desvincular Pet (DELETE /v1/tutores/{id}/pets/{petId})
