...
import { ArrowLeft, Save, PlusCircle, Bug, Camera, Upload } from 'lucide-react';
...
export function PetForm() {
  ...
  async function handleSave(data: PetRequest) {
    try {
      ...
      if (selectedFile && newPet && newPet.id) {
        await petService.uploadPhoto(newPet.id, selectedFile);
      }
      ...
      <div className="bg-surface/40 backdrop-blur-md border border-white/10 rounded-2xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative flex items-center gap-4 mb-8 pb-6 border-b border-white/5">
          ...
          <div>
             ...
             <p className="text-gray-300 text-sm">Preencha os dados para cadastrar um animal</p>
          </div>
        </div>
            ...
            <div className="relative group">
              <div className={`w-40 h-40 rounded-2xl border-2 flex items-center justify-center overflow-hidden transition-all duration-300 ${preview ? 'border-primary shadow-[0_0_20px_rgba(0,230,184,0.3)]' : 'border-dashed border-gray-600 hover:border-primary/50 bg-black/20'}`}>
                {preview ? (
                  ...
                  <Camera className="w-12 h-12 text-gray-400 group-hover:text-primary/80 transition-colors" />
                )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                />
                <p className="text-[10px] text-gray-400 mt-2 flex items-center gap-1.5 opacity-80">
                    <Bug className="w-3 h-3" /> 
                    Campo visual (não salvo no banco)
                </p>
              </div>
